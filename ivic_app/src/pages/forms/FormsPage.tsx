...
export function FormsPage({ data, persist }: FormsPageProps) {
  ...
  const columns: Array<Column<FormRecord>> = [
    { key: "title", header: "名称/摘要", width: "205px", sortable: true, render: (row) => <strong>{row.title}</strong>, sortValue: (row) => row.title },
    ...
    { key: "remark", header: "备注", width: "190px", render: (row) => <span className="cell-ellipsis" title={row.remark}>{shortenText(row.remark, 18)}</span> },
  ...
  function handleExport() {
    ...
    const csv = [
      "名称,票号,票种,金额,购买日期,开票日期,分组,状态,备注",
      ...rows.map((row) => [row.title, row.invoiceNumber, row.invoiceKind, row.amount, displayPurchaseDate(row), row.issueDate, row.groupName, row.status, row.remark].map(csvCell).join(",")),
    ].join("\n");
